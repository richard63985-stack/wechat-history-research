...
这是供 Codex 使用的 **工作流 Skill**，包含指令、数据规范、本地校验和阅读导出脚本；**不是免登录爬虫，也不附带可直接批量抓取任意公众号的采集器**。实际采集由智能体根据可用浏览器、授权和数据源完成，首次使用必须验证目标公众号。
...
上述脚本生成逐篇 MD、目录、数据库快照及 Excel 输入 JSON；**Excel 需再由宿主表格工具生成**，不能把 JSON 改后缀当作 Excel。提供 `@oai/artifact-tool` 的环境可以复用 `build_overview.mjs`，使用步骤见目录规范；此私有运行依赖不随仓库分发，不假定可从 npm 安装。

## 环境与测试边界

- **研究执行**：需要支持本地 Skill 的 Codex，以及相应的浏览器 / 文件 / PDF 能力。安装 Skill 本身不会安装浏览器插件、采集器或模型服务。
- **Python 自检与 MD 导出**：仅依赖标准库，不需要 API Key、不联网，也不读取真实浏览器数据。**Excel builder** 使用宿主提供的 Node.js 与 `@oai/artifact-tool`，缺少该能力时需先解决环境条件。
...
scripts/build_overview.mjs     用宿主表格运行环境生成 Excel
```
